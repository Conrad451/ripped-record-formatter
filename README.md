...
1. **Record the sides** (optional). In the **Record** tab, pick your input, optionally look up the release so the sides are named and the identity travels with them, watch the meters while you set gain, and press Record. Stop at the end of the side, flip the record, press Record again — the file name advances from `SideA.wav` to `SideB.wav` on its own, and each finished side appears in the Full Rip mapping table already assigned. When the record is done, **Done recording — process this album** takes you across to Full Rip with the session in hand. *Or start from an existing folder of WAVs:* in **Full Rip**, select the folder holding this record's side WAVs. One row appears per WAV found. Set the side for each file that belongs to this record and leave the rest on **— skip —** — a folder can hold more than one album, and anything the app isn't sure about it leaves alone rather than guessing.
